...
then
	...
	(sleep 142 && killall -9 gst-launch-1.0 > /dev/null 2>&1)&
	...
	then
		...
		gst-launch-1.0 uridecodebin uri=file:///opt/VideoPlayer/media/Microchip-masters.mp4 expose-all-streams=false name=srcVideo caps="video/x-h264" srcVideo. ! h264parse ! queue ! g1h264dec ! video/x-raw,width=${V_WIDTH},height=${V_HEIGHT} ! drmsink max-lateness=-1 async=false enable-last-sample=false
	fi
fi
